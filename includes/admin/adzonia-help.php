    ...
    if( 'adzonia' === $screen->post_type && in_array($screen->base, array('post', 'edit')) ) {
        ...
        $help_tab_content .= '<ul>';
            $help_tab_content .= '<li>';
                ...
                $help_tab_content .= sprintf( __('<strong>Shortcode</strong> &mdash; The simplest is using a shortcode. The shortcode is that simple, just put %s into the body of any post or page or shortcode enabled widget. Just replace the <em>hash</em> (<code>#</code>) with the ad ID.', 'adzonia'), '<code>[adzonia id="#"]</code>' );
            $help_tab_content .= '</li>';
            $help_tab_content .= '<li>';
        ...
        $screen->add_help_tab( array(
            'id'       => 'adzonia-help',
            'title'    => esc_html__( 'AdZonia', 'adzonia' ),
            'content'  => $help_tab_content
        ));

        // Help sidebar
        $screen->set_help_sidebar(
            '<p><strong>'. esc_html__( 'For more detailed instructions:', 'adzonia' ) .'</strong></p>'.
            '<p><a href="https://github.com/nanodesigns/adzonia/wiki/User-Manual" target="_blank">'. esc_html__( 'AdZonia Manual', 'adzonia' ) .'</a></p>'.
            '<p>'. esc_html__( 'To reduce the plugin size we shifted the manual to our Github Wiki.', 'adzonia' ) .'</p>'
        );
    }
}
